...
public static class TwitchIrcParser
{
    ...
    {
        if (string.IsNullOrWhiteSpace(rawLine) || !rawLine.Contains(" PRIVMSG ", StringComparison.Ordinal))
        {
            return null;
        }
        ...
        var messageIndex = line.IndexOf(" :", StringComparison.Ordinal);
        if (messageIndex < 0)
        {
            return null;
        }
        ...
        var fallbackUser = prefixEnd > 1 && line.StartsWith(':')
            ? line[1..prefixEnd]
            : "unknown";
    ...
    {
        return $"https://static-cdn.jtvnw.net/emoticons/v2/{Uri.EscapeDataString(id)}/default/light/2.0";
    }
