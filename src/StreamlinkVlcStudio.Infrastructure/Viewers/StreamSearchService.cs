...
public sealed class StreamSearchService : IStreamSearchService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var explicitPlatform = IsExplicitPlatformSearch(query, exactCandidates);
        var order = 0;
        ...
        var channels = await Task.WhenAll(distinct.Select(discovery => CreateSearchChannelAsync(
            discovery,
            request.Quality,
            settings,
            customArguments,
        ...
        var orderedChannels = channels
            .OrderBy(channel => MatchRank(channel.Channel, channel.DisplayName, query))
            .ThenBy(channel => channel.State == StreamSearchChannelState.Live ? 0 :
                channel.State == StreamSearchChannelState.Offline ? 1 : 2)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.Write(AppLogLevel.Warning, "Search", "Twitch token validation failed for channel search.", ex);
            return null;
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.Write(AppLogLevel.Warning, "Search", "curl.exe was not found; Kick channel search fallback is unavailable.");
            return null;
        }
    ...
    private static IEnumerable<DiscoveredChannel> ReadTwitchChannels(JsonElement root)
    {
        ...
        foreach (var item in data.EnumerateArray())
        {
            ...
            yield return new DiscoveredChannel(
                PlatformKind.Twitch,
                login,
                FirstNonEmpty(GetOptionalString(item, "display_name"), login),
    ...
    private static bool IsExplicitPlatformSearch(string query, IReadOnlyList<StreamTarget> exactCandidates)
    {
        return exactCandidates.Count == 1 &&
            (query.Contains("://", StringComparison.Ordinal) ||
                query.StartsWith("www.", StringComparison.OrdinalIgnoreCase) ||
                query.StartsWith("twitch.tv/", StringComparison.OrdinalIgnoreCase) ||
                query.StartsWith("kick.com/", StringComparison.OrdinalIgnoreCase));
    }
    ...
    {
        var live = channels.Count(channel => channel.State == StreamSearchChannelState.Live);
        var offline = channels.Count(channel => channel.State == StreamSearchChannelState.Offline);
        var unavailable = channels.Count(channel => channel.State == StreamSearchChannelState.Unavailable);
