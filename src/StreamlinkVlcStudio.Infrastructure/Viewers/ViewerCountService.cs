...
public sealed class ViewerCountService : IViewerCountService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                logger.Write(AppLogLevel.Warning, "Viewers", "Could not resolve Twitch Client ID from the OAuth token.", ex);
                return null;
            }
    ...
    private static ViewerCountResult ReadTwitchViewerCount(StreamTarget target, JsonElement root)
    {
        ...
        foreach (var item in data.EnumerateArray())
        {
            ...
            {
                return new ViewerCountResult(ViewerCountState.Available, viewerCount, "Twitch viewer count updated.");
            }
        }
    ...
    private static ViewerCountResult ReadKickViewerCount(StreamTarget target, JsonElement root)
    {
        ...
        foreach (var item in data.EnumerateArray())
        {
            ...
            {
                return new ViewerCountResult(ViewerCountState.Available, viewerCount, "Kick viewer count updated.");
            }
