...
public sealed class BrowseService : IBrowseService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                logger.Write(AppLogLevel.Warning, "Browse", "Could not resolve Twitch Client ID from the OAuth token.", ex);
                return null;
            }
            ...
            TwitchClientIdCache[tokenKey] = new TwitchClientIdCacheEntry(clientId, expiresAt);
            return clientId;
        }
        finally
        {
    ...
    private static string ReadPaginationCursor(JsonElement root, string propertyName)
    {
