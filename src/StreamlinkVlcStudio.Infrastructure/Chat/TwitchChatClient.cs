...
public sealed class TwitchChatClient : IChatClient, ITwitchPredictionClient
{
    ...
    public async Task<TwitchPrediction?> RefreshPredictionAsync(CancellationToken cancellationToken = default)
    {
        var context = GetPredictionContext();
        var prediction = await predictionApiClient.GetLatestPredictionAsync(
            context.BroadcasterId,
            context.AccessToken,
            context.ClientId,
            cancellationToken);
        if (prediction is { IsOpen: true })
        {
