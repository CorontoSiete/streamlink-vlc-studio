...
/// <summary>
/// Walks Kick's recent-messages cursor pages backwards to fill a time range. Used by the live
/// chat client to seed newly joined chat and to answer history requests.
/// </summary>
internal sealed class KickChatHistoryBackfillService
{
        ...
        ChatHistoryBackfillResult result)
    {
        logger.Write(
            ...
            $"range={KickChatApi.FormatBackfillTimestamp(result.CoveredFromTimestampUtc)} through {KickChatApi.FormatBackfillTimestamp(result.CoveredThroughTimestampUtc)}.");
    }
