...
public sealed class KickChatClient : IChatClient, IChatHistoryBackfillClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var url = BuildKickRecentMessagesUrl(escapedChatroomId, cursor, startTimeUtc: null);
    ...
    private async Task ValidateSendTokenAsync(string token, CancellationToken cancellationToken)
    {
        ...
        if (!tokenInfo.Active)
        {
            if (await TryRefreshAndValidateSendTokenAsync(token, cancellationToken))
            {
                return;
            }
    ...
    private async Task<bool> TryRefreshAndValidateSendTokenAsync(string previousToken, CancellationToken cancellationToken)
    {
    ...
    {
        return KickOAuthService.NormalizeBearerToken(token);
    }
        ...
        string? cursor)
    {
        ...
        yield return "--referer";
        yield return $"https://kick.com/{escapedChannel}";
        yield return BuildKickRecentMessagesUrl(escapedChatroomId, cursor, startTimeUtc: null);
    }
    ...
    private static HashSet<string> ReadScopes(JsonElement data)
    {
        var scopes = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        if (!data.TryGetProperty("scope", out var scopeProperty))
        {
            return scopes;
        }
