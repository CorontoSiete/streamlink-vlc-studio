...
internal sealed class DockedChatBadgeCatalog
{
    ...
    private async Task LoadTwitchBadgesAsync(string url, string? roomId = null)
    {
            ...
            badgeSets.ValueKind == JsonValueKind.Object)
        {
            foreach (var badgeSet in badgeSets.EnumerateObject())
            {
                ...
                foreach (var version in versions.EnumerateObject())
                {
                    ...
                    var title = TryGetString(version.Value, "title", out var parsedTitle)
                        ? parsedTitle
        ...
        if (normalizedRoomId.Length > 0 && !HasTwitchSubscriberVersions(normalizedRoomId))
        {
            changed |= await LoadTwitchHelixChannelBadgesAsync(normalizedRoomId).ConfigureAwait(false);
        }
    ...
    private async Task<bool> LoadTwitchHelixChannelBadgesAsync(string roomId)
    {
        using var document = await TryGetTwitchHelixJsonAsync($"/helix/chat/badges?broadcaster_id={Uri.EscapeDataString(roomId)}").ConfigureAwait(false);
        if (document is null ||
        ...
        foreach (var badgeSet in data.EnumerateArray())
        {
            if (badgeSet.ValueKind != JsonValueKind.Object ||
                !TryGetString(badgeSet, "set_id", out var id) ||
            ...
            foreach (var version in versions.EnumerateArray())
            {
                if (version.ValueKind != JsonValueKind.Object ||
                    !TryGetString(version, "id", out var versionId) ||
                    !TryGetPreferredBadgeImageUrl(version, out var imageUrl))
                {
                    continue;
                }

                var title = TryGetString(version, "title", out var parsedTitle)
                    ? parsedTitle
    ...
    private async Task LoadKickChannelBadgesAsync(string channel)
    {
        ...
        {
            var months = TryGetInt(subscriberBadge, "months", 0);
            if (months <= 0 ||
    ...
    {
        var manifestPath = BundledBadgeAssets.FindTwitchBadgeManifestPath();
        ...
        try
        {
            ...
            foreach (var entry in entries.EnumerateArray())
            {
                if (!TryGetString(entry, "id", out var id) ||
                    !TryGetString(entry, "version", out var version) ||
                    !TryGetString(entry, "image", out var relativeImagePath))
                {
                    continue;
                }
                ...
                var title = TryGetString(entry, "title", out var parsedTitle)
                    ? parsedTitle
                    : GetBadgeToolTip(new ChatBadge(id, version));
                changed |= AddTwitchBadge(null, id, version, title, new Uri(imagePath).AbsoluteUri);
            }
        ...
        try
        {
            ...
            foreach (var entry in entries.EnumerateArray())
            {
                ...
                var title = TryGetString(entry, "title", out var parsedTitle)
                    ? parsedTitle
                    : GetBadgeToolTip(new ChatBadge(id, version));
                changed |= AddKickBadge(null, id, version, title, new Uri(imagePath).AbsoluteUri);
            }
    ...
    private bool HasTwitchSubscriberVersions(string roomId)
    {
        var versionKey = MakeTwitchBadgeVersionKey(roomId, "subscriber");
        lock (sync)
        {
            return twitchNumericBadgeVersions.TryGetValue(versionKey, out var versions) && versions.Count > 0;
        }
    }
    ...
    {
        return TryGetString(version, "image_url_2x", out imageUrl) ||
            TryGetString(version, "image_url_4x", out imageUrl) ||
            TryGetString(version, "image_url_1x", out imageUrl);
    }
    ...
    private static bool TryGetKickSubscriberBadgeImageUrl(JsonElement item, out string imageUrl)
    {
        if (item.ValueKind == JsonValueKind.Object)
        {
            if (item.TryGetProperty("badge_image", out var badgeImage) &&
                ...
                TryGetString(badgeImage, "src", out imageUrl))
            {
                return true;
            }

            if (TryGetString(item, "image_url", out imageUrl) ||
                TryGetString(item, "imageUrl", out imageUrl) ||
                TryGetString(item, "src", out imageUrl))
            {
                return true;
            }
        }
