...
public partial class MainWindow : Window
{
    ...
    {
        hasActiveLowLevelMouseMoveRoute = tabDetachDragTab is not null || videoReorderDragTab is not null;
    }
    ...
    internal bool TryMergeTabsIntoMultiViewTab(StreamTabViewModel draggedTab, StreamTabViewModel targetTab)
    {
        return TryMergeTabsIntoMultiViewTabs([draggedTab], draggedTab, targetTab);
    }
    ...
    internal bool TryAttachTabsToPictureInPictureTab(StreamTabViewModel draggedTab, StreamTabViewModel targetTab)
    {
        if (viewModel is null ||
            !viewModel.Tabs.Contains(draggedTab) ||
            !detachedWindows.TryGetValue(targetTab, out var targetWindow) ||
            targetWindow.IsClosing)
        {
            return false;
        }

        var draggedTabs = GetPictureInPictureDragTabs(draggedTab);
        return AddTabsToPictureInPictureWindow(targetWindow, draggedTabs, draggedTab);
    }
    ...
    private void DetachTabToPictureInPicture(StreamTabViewModel tab, Point screenPoint, bool continueDrag)
    {
        ...
        var detachedWindow = new DetachedVideoWindow(detachedTabs, tab);
    ...
    private bool PositionDetachedWindow(DetachedVideoWindow window, Point screenPoint, bool useSavedLocation)
    {
        ...
        var dipPoint = ToDeviceIndependentPoint(screenPoint);
        ...
        out Rect workingArea)
    {
        ...
        workingArea = ToDeviceIndependentRect(new NativeRectangle
        {
    ...
    private void ApplyMonitorMaxInfo(IntPtr hwnd, IntPtr minMaxInfoPointer)
    {
        ...
        var targetBounds = fullscreen ? monitorInfo.Monitor : monitorInfo.WorkArea;
    ...
    private Rect GetCurrentMonitorBounds(bool useWorkingArea)
    {
        ...
        return ToDeviceIndependentRect(useWorkingArea ? monitorInfo.WorkArea : monitorInfo.Monitor);
    }
    ...
    private Rect GetMonitorWorkingAreaAtScreenPoint(Point screenPoint)
    {
        ...
        return GetMonitorInfo(monitor, ref monitorInfo)
            ? ToDeviceIndependentRect(monitorInfo.WorkArea)
            : SystemParameters.WorkArea;
    }

    private Rect GetMonitorWorkingAreaAtDeviceIndependentPoint(Point point)
    {
        return GetMonitorWorkingAreaAtScreenPoint(ToDevicePoint(point));
    }

    private bool TryGetMonitorInfo(IntPtr hwnd, out MonitorInfo monitorInfo)
    {
    ...
    private Point ToDeviceIndependentPoint(Point screenPoint)
    {
    ...
    private Rect ToDeviceIndependentRect(NativeRectangle rectangle)
    {
        var transform = PresentationSource.FromVisual(this)?.CompositionTarget?.TransformFromDevice ?? Matrix.Identity;
