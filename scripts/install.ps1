...
function Resolve-AppPayloadRoot([string]$ExtractDirectory) {
...
function Install-AppFromGitHubRelease {
    ...
    $downloadPath = Join-Path $script:TempRoot $asset.name
...
function Install-AppFromGitHubArtifact {
    ...
    $downloadPath = Join-Path $script:TempRoot ($artifact.name + ".zip")
...
function Ensure-LatestVlc {
    ...
    $downloadPath = Join-Path $script:TempRoot $latest.FileName
    New-Item -ItemType Directory -Path $script:TempRoot -Force | Out-Null
