internal static partial class ApplicationTestCatalog
{
    ...
    ("sends Twitch VOD chat GraphQL offsets as integers", () =>
    {
        var method = typeof(TwitchVodChatFetcher).GetMethod(
            "BuildPayload",
            BindingFlags.NonPublic | BindingFlags.Static);
        Assert.NotNull(method);
        ...
        var payload = (string)method!.Invoke(
            null,
            ["2786354640", TimeSpan.FromSeconds(5971.6437039)])!;
        using var document = JsonDocument.Parse(payload);
        var variables = document.RootElement[0].GetProperty("variables");
        var offset = variables.GetProperty("contentOffsetSeconds");
        ...
        // Cursor paging is rejected by Twitch without a Client-Integrity token, so the request
        // must never carry one.
        Assert.Equal(false, variables.TryGetProperty("cursor", out _));
        return Task.CompletedTask;
    }),
    ];
}
